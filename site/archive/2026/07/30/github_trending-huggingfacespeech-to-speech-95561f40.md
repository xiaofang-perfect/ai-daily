...
date: 2026-07-30
published_at: 2026-07-30T04:55:26.998764+00:00
tag: 工具开源
item_id: 95561f40ccb1a407
---
...
This pipeline runs in production as the conversation backend for thousands of [Reachy Mini](https://huggingface.co/blog/reachy-mini) robots.
  ...
  ![Switching an OpenAI Realtime client endpoint from hosted OpenAI to a self-hosted speech-to-speech server](https://github.com/huggingface/speech-to-speech/raw/main/docs/assets/endpoint-swap-light.gif) 
...
`python scripts/listen_and_play_realtime.py --host 127.0.0.1 --port 8765`Prefer to keep the LLM on your own machine? Serve Gemma 4 with llama.cpp:

`llama-server -hf ggml-org/gemma-4-E4B-it-GGUF -np 2 -c 65536 -fa on --swa-full`Then point the OpenAI-compatible LLM backend at it:
...
```
Any OpenAI Realtime-compatible client can connect. See [Realtime API](https://github.com#realtime-api) for the protocol and [LLM backends](https://github.com#llm-backends) for provider and local-server options.

- [How it works](https://github.com#how-it-works)
- [Installation](https://github.com#installation)
- [Supported components](https://github.com#supported-components)
- [Run modes](https://github.com#run-modes)
- [Realtime API](https://github.com#realtime-api)
- [LLM backends](https://github.com#llm-backends)
- [Multi-language support](https://github.com#multi-language-support)
- [Pocket TTS](https://github.com#pocket-tts)
- [CLI reference](https://github.com#cli-reference)
- [Contributing](https://github.com#contributing)
- [Star history](https://github.com#star-history)
- [Citations](https://github.com#citations)
...
`pip install speech-to-speech`The default install covers the standard realtime path:
...
```
Deprecated implementations, including MeloTTS, live in [ archive/](https://github.com/huggingface/speech-to-speech/blob/main/archive) and are no longer wired into the CLI.
...
`speech-to-speech --local_mac_optimal_settings`Optionally with a specific LLM:
...
```
- 
Run the pipeline in WebSocket mode: speech-to-speech --mode websocket --ws_host 0.0.0.0 --ws_port 8765 
- 
...
- 
Run the pipeline on the server: speech-to-speech --mode socket --recv_host 0.0.0.0 --send_host 0.0.0.0 
- 
Run the client locally to handle microphone input and playback: python scripts/listen_and_play.py --host <IP address of your server> 
...
`docker compose up`The compose file starts a llama.cpp server with Gemma 4, starts the TCP socket server, and exposes ports `8080`, `12345`, and `12346`.
...
with client.realtime.connect(model="local") as conn:
    ...
    for event in conn:
        print(event.type)
```
The server implements the core Realtime event set: `input_audio_buffer.append`, `session.update`, `conversation.item.create`, `response.create`, and `response.cancel` inbound; speech start/stop, streaming transcription, audio deltas, tool calls, and `response.done` outbound. The full event reference, architecture, and design details live in the [Realtime Engine README](https://github.com/huggingface/speech-to-speech/blob/main/src/speech_to_speech/api/openai_realtime/README.md).
...
References for all CLI arguments live in the [arguments classes](https://github.com/huggingface/speech-to-speech/blob/main/src/speech_to_speech/arguments_classes) and in `speech-to-speech -h`.

See [ModuleArguments](https://github.com/huggingface/speech-to-speech/blob/main/src/speech_to_speech/arguments_classes/module_arguments.py). It allows setting:
...
See [VADHandlerArguments](https://github.com/huggingface/speech-to-speech/blob/main/src/speech_to_speech/arguments_classes/vad_arguments.py). Notable options:
...
```
Citations for optional backends such as Kokoro, Pocket TTS, ChatTTS, Whisper variants, Paraformer, and MMS live in the respective [component READMEs](https://github.com/huggingface/speech-to-speech/blob/main/src/speech_to_speech).
