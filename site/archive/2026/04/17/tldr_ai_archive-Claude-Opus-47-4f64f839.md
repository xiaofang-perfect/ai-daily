...
url: https://www.anthropic.com/news/claude-opus-4-7?utm_source=tldrai
date: 2026-04-17
published_at: 2026-04-17T12:00:00+00:00
tag: 产品发布
item_id: 4f64f8390ba0edbd
---
...
*More effort control*: Opus 4.7 introduces a new`xhigh`

(“extra high”)[effort level](https://platform.claude.com/docs/en/build-with-claude/effort)between`high`

and`max`

, giving users finer control over the tradeoff between reasoning and latency on hard problems. In Claude Code, we’ve raised the default effort level to`xhigh`

for all plans. When testing Opus 4.7 for coding and agentic use cases, we recommend starting with`high`

or`xhigh`

effort.*On the Claude Platform (API)*: as well as support for higher-resolution images, we’re also launching task budgets in public beta, giving developers a way to guide Claude’s token spend so it can prioritize work across longer runs.*In Claude Code*: The new`/ultrareview`
