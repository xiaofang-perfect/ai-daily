...
date: 2026-06-23
published_at: 2026-06-23T06:03:33.873345+00:00
tag: 工具开源
item_id: 8ccdd7d5cb1ca232
---
![OpenMontage](https://github.com/calesthio/OpenMontage/raw/main/assets/logo.png)
...
NoRun manually:`make`?`pip install -r requirements.txt && cd remotion-composer && npm install && cd .. && pip install piper-tts && cp .env.example .env`
...
- **Read the contract first**Start with- `AGENT_GUIDE.md`- `PROJECT_CONTEXT.md`
...
- **Check the actual capability envelope**Run:- python -c "from tools.tool_registry import registry; import json; registry.discover(); print(json.dumps(registry.support_envelope(), indent=2))" python -c "from tools.tool_registry import registry; import json; registry.discover(); print(json.dumps(registry.provider_menu(), indent=2))" 
...
`make demo` to render zero-key demo videos instantly.Each pipeline is a complete production workflow, from idea to finished video.
...
- The registry auto-discovers it — no manual registration needed
...
- [Show and Tell](https://github.com/calesthio/OpenMontage/discussions/categories/show-and-tell)
- [Ideas](https://github.com/calesthio/OpenMontage/discussions/categories/ideas)
- [Q&A](https://github.com/calesthio/OpenMontage/discussions/categories/q-a)
