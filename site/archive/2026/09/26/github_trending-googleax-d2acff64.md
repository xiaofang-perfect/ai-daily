...
date: 2026-09-26
published_at: 2026-09-26T07:00:26.573682+00:00
tag: 工具开源
item_id: d2acff646b4969f5
---
# 
  
  AX
![AX axolotl](/google/ax/raw/main/assets/axolotl.svg)
...
`kubectl get svc api -n ate-system``go install github.com/google/ax/cmd/ax@latest`
This puts the `ax` binary in `$(go env GOPATH)/bin`. Make sure that directory is on your `PATH`.

With the [prerequisites](https://github.com#prerequisites) in place — most importantly a reachable Agent Substrate Control API — deploy the AX control plane:
...
| Guide | Read it to... | 
|---|---|
| [Concepts](https://github.com/google/ax/blob/main/docs/concepts.md) | Learn what a `Task` ,`Workspace` , and`Model` each do, and how a task moves through phases and conditions. | 
...
| [Roadmap](https://github.com/google/ax/blob/main/docs/roadmap.md) | See planned milestones across core specs, actor architecture, agentic environments, and governance. | 
