---
title: "Ideogram 4 (GitHub Repo)"
source: TLDR AI · 2026-06-04
url: https://github.com/ideogram-oss/ideogram4?utm_source=tldrai
date: 2026-06-05
published_at: 2026-06-04T12:00:00+00:00
tag: 工具开源
item_id: 110ad88bc8db7bd5
---
*Ideogram 4: Open image model at the forefront of design*

Ideogram 4 is ** Ideogram's first open-weight text-to-image model**. It is a

**state-of-the-art foundation model trained from scratch**— not a fine-tune of any existing model. It introduces a new structured JSON prompting interface, with best-in-class multilingual text rendering, deep language understanding, explicit bounding-box layout and color-palette controls, and native 2k resolution images. The easiest way to try the model is online at

**.**

[ideogram.ai](https://ideogram.ai/)We believe openness drives innovation, and we invite the research community to innovate with us on the forefront of visual intelligence.

**[2026-06-03]****Ideogram 4 released!**Inference code and weights are now public, and our[technical blog post](https://ideogram.ai/blog/ideogram-4.0/)is live. See the[Quick Start](https://github.com#quick-start)section to generate your first image, or try the model online at[ideogram.ai](https://ideogram.ai/).
...
[Ideogram 4 (fp8)](https://huggingface.co/ideogram-ai/ideogram-4-fp8)[Ideogram 4 Non-Commercial](https://github.com/ideogram-oss/ideogram4/blob/main/model_licenses/LICENSE-IDEOGRAM-4-NON-COMMERCIAL)We plan to support more quantizations in the future.
...
[ideogram-ai/ideogram-4-nf4](https://huggingface.co/ideogram-ai/ideogram-4-nf4)(or[ideogram-ai/ideogram-4-fp8](https://huggingface.co/ideogram-ai/ideogram-4-fp8)) — and click**Agree and access repository**to accept the license gate. -
Create a Hugging Face access token at

[huggingface.co/settings/tokens](https://huggingface.co/settings/tokens)and log in so the download is authenticated:hf auth login
...
is rewritten into the structured JSON caption the model
...
model or system prompt needed). It reads `IDEOGRAM_API_KEY`

— get a key at
[https://developer.ideogram.ai/](https://developer.ideogram.ai/):
...
For the highest-quality images, set `--height 2048 --width 2048`
...
then export them as `HIVE_TEXT_MODERATION_KEY`

and `HIVE_VISUAL_MODERATION_KEY`

(or pass them via `--hive-text-key`
...
**Use JSON prompts**for maximum controllability — the model was trained on them and understands the structure natively.**Color palette conditioning**— specify a`colour_palette`

array of hex colors in the style description to steer the image's color scheme.**Aspect ratio flexibility**— Ideogram 4 supports a wide range of aspect ratios (any multiple-of-16 resolution from 256 to 2048 on each side). This is a key advantage for practical use: portraits, landscapes, banners, phone wallpapers, social media formats, etc.**Bounding-box layout**— specify`bbox`

coordinates in the prompt to explicitly place subjects, text elements, and background regions.**Compositional control**— use`compositional_deconstruction`

with bounding boxes and per-element descriptions for precise spatial layout.
...
default in `run_inference.py`

(see the [CLI](https://github.com#cli) section).
...
[docs/inference.md](https://github.com/ideogram-oss/ideogram4/blob/main/docs/inference.md)[docs/model_architecture.md](https://github.com/ideogram-oss/ideogram4/blob/main/docs/model_architecture.md)[docs/pipeline.md](https://github.com/ideogram-oss/ideogram4/blob/main/docs/pipeline.md)[docs/development.md](https://github.com/ideogram-oss/ideogram4/blob/main/docs/development.md)[docs/safety.md](https://github.com/ideogram-oss/ideogram4/blob/main/docs/safety.md)If you find the provided code or models useful for your research, consider citing them as:
...
them. Interested candidates please apply [https://jobs.ashbyhq.com/ideogram](https://jobs.ashbyhq.com/ideogram)
